...
const trackVisitor = async () => {
    ...
    if (!visited) {
        try {
            // Total Visitors
            await db.collection('metrics').doc('visitors').set({
                total: firebase.firestore.FieldValue.increment(1),
                lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
            }, { merge: true });

            // Daily Visitors (Optional - simple key based on date)
            const today = new Date().toISOString().split('T')[0];
            await db.collection('metrics').doc(`visitors_${today}`).set({
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.loadAdminStats = async () => {
        const visitorTotalEl = document.getElementById('stats-visitor-count');
        const visitorTodayEl = document.getElementById('stats-today-count');
        const rankingEl = document.getElementById('stats-booklet-ranking');

        if (!visitorTotalEl || !db) return;

        try {
            // 1. Load Visitors
            const totalDoc = await db.collection('metrics').doc('visitors').get();
            const today = new Date().toISOString().split('T')[0];
            const todayDoc = await db.collection('metrics').doc(`visitors_${today}`).get();

            visitorTotalEl.textContent = totalDoc.exists ? (totalDoc.data().total || 0).toLocaleString() : '0';
            visitorTodayEl.textContent = todayDoc.exists ? (todayDoc.data().count || 0).toLocaleString() : '0';

            // 2. Load Top Booklets (Client-side sort for simplicity as clicks might be sparse)
            const snapshot = await db.collection('posts')
                .where("tags", "array-contains", "전도 소책자 PDF")
                .get();
            ...
            snapshot.forEach(doc => {
                const data = doc.data();
                if (data.clicks) {
                    booklets.push({ id: doc.id, title: data.title, clicks: data.clicks });
                }
            });

            // Sort by clicks desc
            booklets.sort((a, b) => b.clicks - a.clicks);

            // Take Top 10
            const top10 = booklets.slice(0, 10);

            if (top10.length === 0) {
                rankingEl.innerHTML = '<div style="text-align:center; color:#999;">아직 집계된 조회수가 없습니다.</div>';
            } else {
                let html = '<ul style="list-style:none; padding:0;">';
                top10.forEach((item, index) => {
                    html += `
                        <li style="display:flex; justify-content:space-between; padding:10px; border-bottom:1px solid #eee; font-size:0.9rem;">
                            <span><strong style="color:var(--secondary-color); margin-right:10px;">${index + 1}위</strong> ${item.title}</span>
                            <span style="font-weight:bold; color:#555;">${item.clicks.toLocaleString()}회</span>
                        </li>
                    `;
                });
                html += '</ul>';
            ...
            visitorTotalEl.textContent = "Error";
        }
    };
        ...
        uploadForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const postData = {
                    ...
                    fileUrl,
                    coverUrl,
                    videoUrl: videoUrl, // Add videoUrl field
            ...
            if (typeof isAdmin !== 'undefined' && isAdmin) {
                ...
                if (modalUploadForm) {
                    modalUploadForm.onsubmit = async (e) => {
                        ...
                        const videoUrl = document.getElementById('modal-post-video').value.trim() || "";
